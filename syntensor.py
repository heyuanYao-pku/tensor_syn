...
def SynTensor:
    def __init__(self, n, mlist, Plist):
        ...
        assert size(Plist) == (n,n), 'Size of Plist must be (n,n)'
        ...
        buildC()
        buildR()
        self.Ax,self.Bx,self.Cx = get_init()

    def buildP(self):
        self.P = np.zeros([self.N,self.N],np.int)
        for i in range(n):
            for j in range(n):
                self.P[indBegin[j]:indEnd[j],indBegin[i]:indEnd[i]] = self.Plist[i][j]
    ...
    def buildC(self):
        ...
        for i in range(n):
            for j in range(n):
                for k in range(n):
                    tmp = np.dot(Plist[j][k],Plist[i][j])
                    tmp = np.dot(Plist[k][i],tmp)
                    d = np.diag(tmp)
                    d = np.diag(d)
